...
class SDFNetNaruto(SDFNet):
    ...
    def forward(self, x, return_geo=True):
        if self.config['multi_agents']['track_uncertainty']:
            x = x.float()
            uncert, x = x[:, 0], x[:, 1:]
            out = self.model(x) # [..., 17]
            # out = torch.cat([out, uncert[..., None]], dim=1) # [..., 17+1]
        else:
            out = self.model(x)

        if return_geo:  # return feature
            return out #[..., 18(17+1)], now #[..., 17]
        else:
            return out[..., :1]

    def get_model(self, config):
        tcnn_network = config['decoder']['tcnn_network']
        uncert = 1 if config['multi_agents']['track_uncertainty'] else 0
        if tcnn_network:
            print('SDF net: using tcnn')
            ...
            #print("n_output_dims", 1 + self.geo_feat_dim + uncert)
            sdf_net = []
            for l in range(self.num_layers):
                if l == 0:
                    in_dim = self.input_ch
...
class ColorSDFNet_v2_Naruto(nn.Module):
    ...
    def __init__(self, config, input_ch=3, input_ch_pos=12):
        ...
        self.config = config
        self.pred_uncert = config['multi_agents']['track_uncertainty']
        self.color_net = ColorNet(config, 
                input_ch=input_ch_pos, 
                geo_feat_dim=config['decoder']['geo_feat_dim'], 
                hidden_dim_color=config['decoder']['hidden_dim_color'], 
    ...
    def forward(self, embed, embed_pos):
        ...
        if embed_pos is not None:
            h = self.sdf_net(torch.cat([embed, embed_pos], dim=-1), return_geo=True)  #[..., 18(17+1)], now [..., 17(16+1)]
        else:
            h = self.sdf_net(embed, return_geo=True) 
        
        sdf, geo_feat = h[...,:1], h[...,1:] # [..., 1] and [..., 17(16+1)], now the latter one is [..., 16(15+1)]
        if self.pred_uncert:
            geo_feat, uncert = geo_feat[..., :-1], geo_feat[..., -1:]  #[..., 16] and [..., 1], now the first one is [..., 15(geo_feat_dim)]
            # print("feat dim:", geo_feat.shape, "uncert dim:", uncert.shape)
